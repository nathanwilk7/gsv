...
class SplitRead(object):
    ...
    def is_valid(self,
        ...
        # check for SA tag
        if not self.read.has_tag('SA'): # I think a read with an SA tag is part of a chimeric alignment?
            # Include soft-clipped reads that didn't generate split-read alignments
            # Check if the read was clipped
            if is_clip_op(self.read.cigar[0][0]) or is_clip_op(self.read.cigar[-1][0]):
                # Get the number of clipped bases on the end of the read
                ...
                # If there are less than x bases that were not aligned
...
def gather_reads(sample,
    ...
    # Get the query bounds for the BAM
    query_bounds = bam_query_bounds(pos, ci, fetch_flank, chrom_length)

    # Get the reads from the BAM
    for read in sample.bam.fetch(chrom, query_bounds[0], query_bounds[1]):
...
def sv_genotype(bam_string,
    ...
    if lib_info_path is not None and os.path.isfile(lib_info_path):
        lib_info = parse_lib_info_path(lib_info_path)

    # Complain if there's not an input VCF file
    if vcf_in is None:
    ...
    sample_list = build_sample_libraries(bam_list, lib_info, lib_info_path, num_samp, min_lib_prevalence, min_aligned)
    sys.stderr.write(' done\n')

    # diagnostic dump of relevant BAM reads
    if alignment_outpath is not None:
        out_bam = get_out_bam(bam_string, alignment_outpath)

    # write the JSON for each sample's libraries
    if lib_info_path is not None and not os.path.isfile(lib_info_path): # TODO: bug - This shouldn't have the not on isfile, right?
        write_sample_json_wrapper(lib_info_path, sample_list)
    ...
    for line in vcf_in:
        if in_header:
            if line[0] == '#':
                ...
                # Format the headers of the vcf file
                format_vcf(vcf, header)

                # add the samples in the BAM files to the VCF output
                add_samples_to_vcf(vcf, sample_list)

                # write the output header
                vcf_out.write(vcf.get_header() + '\n')

        var = Variant(line, vcf)
        ...
        if svtype not in ('BND', 'DEL', 'DUP', 'INV'):
            # Skip this svtype and write variant to vcf_out
            skip_unsupported_svtype(var, svtype, vcf_out)
            continue

        # TODO: Come back to this after I've learned what a breakend is...
        if svtype == 'BND':
...
# TODO: Make this oop-style?
def parse_lib_info_path (lib_info_path):
    """
    Returns the JSONified library information for the given JSON library info file.
    """
    lib_info_file = open(lib_info_path, 'r')
    return json.load(lib_info_file) 

def build_sample_libraries (bam_list, lib_info, lib_info_path, num_samp, min_lib_prevalence, min_aligned):
    """
    Returns a list of all the samples from the list of BAM files.
    """
    sample_list = list()
...
def get_out_bam (bam_string, alignment_outpath):
    """
    Return a BAM file whose header is based on the first input BAM.
    """
...
def write_sample_json_wrapper (lib_info_path, sample_list):
    """
    Write json for sample/library to file.
    """
...
def format_vcf (vcf, header):
    """
    Format the header of the vcf file that will be used as the output file.
    """
...
def add_samples_to_vcf (vcf, sample_list):
    """
    Add the samples in the sample_list to the vcf
    """
    for sample in sample_list:
        if sample.name not in vcf.sample_list:
            vcf.add_sample(sample.name)

def skip_unsupported_svtype (var, svtype, vcf_out):
    """
    This writes an error to notify of skipped variant and writes the skipped variant to the vcf_out
    """
    sys.stderr.write('Warning: Unsupported SV type at variant %s (%s). Skipping.\n' % (var.var_id, svtype))
    vcf_out.write(var.get_var_string() + '\n')


def bam_query_bounds (pos, ci, fetch_flank, chrom_length):
    return (max(pos + ci[0] - fetch_flank, 0), min(pos + ci[1] + fetch_flank + 1, chrom_length))
