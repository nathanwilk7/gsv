...
predicted_gts = []
natee = -1
with open('features3.csv', 'r') as f:
    for line in f:
        if natee == -1:
            natee += 1
            continue
        ...
        svtype = features[0]
        genotype = features[1]
        slb = float(features[2])
        srb = float(features[3])
        cbb = float(features[4])
        mosv = float(features[5])
        cd = float(features[6])
        s = float(features[7])
        rs = float(features[8])
        ...
        if is_dup:
            dups.add(natee)
        natee += 1
        output_stuff.append((svtype, genotype, slb, srb, cbb, mosv, cd, s, rs))
